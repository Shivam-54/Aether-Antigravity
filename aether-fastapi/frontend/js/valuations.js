...
function updatePropertyCounts() {
    // Active = Owned + Rented
    const activeCount = allProperties.filter(p => p.status === 'Owned' || p.status === 'Rented').length;
...
async function renderPropertyFolders() {
    ...
    if (currentFilter === 'Active') {
        ...
        filteredProperties = allProperties.filter(p => p.status === 'Owned' || p.status === 'Rented');
    } else {
        // Sold
        filteredProperties = allProperties.filter(p => p.status === 'Sold');
    }
...
async function openPropertyDetail(propertyId) {
    currentPropertyForDetail = allProperties.find(p => p.id === propertyId);
